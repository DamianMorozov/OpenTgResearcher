...
public sealed class TgEfLicenseRepository : TgEfRepositoryBase<TgEfLicenseEntity, TgEfLicenseDto>, ITgEfLicenseRepository
{
    ...
    #region Methods - ITgEfLicenseRepository

    /// <inheritdoc />
    public async Task SaveAsync(TgLicenseDto dto, CancellationToken ct = default)
    {
        var efLicenseDto = new TgEfLicenseDto();
        efLicenseDto.LicenseKey = dto.LicenseKey;
        efLicenseDto.UserId = dto.UserId;
        efLicenseDto.LicenseType = dto.LicenseType;
        efLicenseDto.ValidTo = dto.ValidTo;
        efLicenseDto.IsConfirmed = dto.IsConfirmed;
        await SaveAsync(efLicenseDto, ct);
    }

    /// <inheritdoc />
    public async Task SaveAsync(TgEfLicenseDto dto, CancellationToken ct = default)
    {
        TgEfLicenseEntity? entity = null;
        try
        {
            if (dto is null)
                return;
            ...
            if (entity is null)
                // Find by LicenseKey
                entity = await GetQuery(isReadOnly: false).SingleOrDefaultAsync(x => x.LicenseKey == dto.LicenseKey, ct);
        ...
        {
            TgLogUtils.WriteException(ex, "Error saving license");
            throw;
        }
        finally
        {
    ...
    /// <inheritdoc />
    public async Task SaveListAsync(IEnumerable<TgLicenseDto> dtos, CancellationToken ct = default)
    {
        try
        {
        ...
        {
            TgLogUtils.WriteException(ex, "Error saving licenses");
            throw;
        }
    }

    /// <inheritdoc />
    public async Task SaveListAsync(IEnumerable<TgEfLicenseDto> dtos, CancellationToken ct = default)
    {
        try
        {
            foreach (var dto in dtos)
