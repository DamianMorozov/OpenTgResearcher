...
internal partial class TgMenuHelper
{
    ...
    private async Task FillTableRowsStorageAsync(TgDownloadSettingsViewModel tgDownloadSettings, Table table)
    {
        ...
        await FillTableRowsStorageTableAsync(table, BusinessLogicManager.StorageManager.ChatUserRepository, TgLocale.MenuTableChatUsersRecordsCount);
    ...
    private async Task FillTableRowsClientConDownloadAsync(TgDownloadSettingsViewModel tgDownloadSettings, Table table)
    {
        ...
        // Enable parsing comments
        table.AddRow(GetMarkup(TgLocale.InfoMessage(TgLocale.MenuDownloadSetIsParsingComments)),
            GetMarkup(tgDownloadSettings.SourceVm.Dto.IsParsingComments.ToString()));
        ...
        // Subscribe
        table.AddRow(GetMarkup(TgLocale.InfoMessage(TgLocale.MenuDownloadSetIsSubscribe)),
            GetMarkup(tgDownloadSettings.SourceVm.Dto.IsSubscribe.ToString()));
